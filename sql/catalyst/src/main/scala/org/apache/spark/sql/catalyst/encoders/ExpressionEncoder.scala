 ...
 * and a `deserializer`.
 *
 * @param serializer An expression that can be used to encode a raw object to corresponding
 ...
 *                   type `T`.
 * @param deserializer An expression that will construct an object given a Spark SQL
...
//   */
  def isSerializedAsStruct: Boolean = serializer.isInstanceOf[StructType]
